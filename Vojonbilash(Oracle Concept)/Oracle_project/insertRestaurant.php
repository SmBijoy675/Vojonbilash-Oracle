<?php  
 include 'adminDesign.php';

 $error='';
 $name = $category = $mobile = $location= ''; 
 $nameErr = $catErr = $numErr = $locationErr = ''; 
 if(isset($_POST["submit"]))  
 {  
     $name = $_POST['name'];
     $category = $_POST['category'];
     $number = $_POST['number'];
     $location = $_POST['location'];

     if(empty($_POST["name"]))  
     {  
          $nameErr = "Enter Name";  
     }
     else if (empty($_POST["category"])) 
     {
          $catErr = "Category is required";
     }
     else if(empty($_POST["number"]))  
     {  
          $numbErr = "Enter number";  
     }
     else if(empty($_POST["location"]))  
     {  
          $numberErr = "Enter location";  
     }
      
     else{
        $conn = oci_connect('scott', 'tiger', 'localhost/XE');
    
        $sql = oci_parse($conn,"INSERT INTO restaurantData (NAME, CATEGORY, LOCATION, MOBILE) 
                VALUES ('{$name}', '{$category}','{$location}','{$number}' )");
        $result = oci_execute($sql);
        if ($result) {
                    header("location:insertionSuccess.php");
                    exit();
        }
        else{
            echo "Error !";
                    exit();
        }
     }
    }  

 ?>  

<html>
<head>
           <title>VojonBilash</title>  
           <script src="https://ajax.googleapis.com/ajax/libs/jquery/2.2.0/jquery.min.js"></script>  
           <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.6/css/bootstrap.min.css" />
           <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.6/js/bootstrap.min.js"></script>  
      </head> 
      <style>
     .error 
     {
          color: #FF0000;
     }
     .heading
     {
          color: coral;
     }
     </style> 
     
      <body>  
           <br><br><br><br><br>  
           <div class="container" style="width:500px;">  
                                
                <form method="post">  
                     <?php   
                     if(isset($error))  
                     {  
                          echo $error;  
                     }  
                     ?>  
                     <br /> 
                     <fieldset>
                    <legend class="heading">INSERT NEW RESTAURANT</legend> 
                     <label>Name</label>  
                     <input type="text" name="name" class="form-control"  />
                     <span class="error">* <?php echo $nameErr;?></span> <br /> 

                     <label>Category</label>
                     <input type="text" name = "category" class="form-control" />
                     <span class="error">* <?php echo $catErr;?></span><br />

                    <label>Location</label>  
                    <input type="text" name="location" class="form-control" />
                    <span class="error">* <?php echo $locationErr;?></span><br />

                     <label>Mobile</label>
                     <input type="text" name = "number" class="form-control" />
                     <span class="error">* <?php echo $numErr;?></span><br />
                    

                    
                     
                    <input type="submit" name="submit" value="Submit"/>
                    <input type="submit" name="reset" value="Reset" /><br />           
                    </fieldset> 

               </form>  
          </div>  
          <br />  
     </body>  
</html>